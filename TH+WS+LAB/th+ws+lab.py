#applica il metodo otsu e il watershed e il labelling, mostra le associazioni tra "auto" e "thruth" con le frecce, calcola i valori medi e gli errori per i parametri di valutazione dell'accuratezza
...
for radius_input in range(3,4): #Viene eseguito il programma per i valori del raggio (della mediana) per i valori in range
    for min_distance_input in range(30,31): #Viene eseguito il programma per i valori della distanza (della ricerca del minimo) per i valori in range
        ...
        for element in list_file_names:
            ...
            path_R=path[:-49]+'-labels.tif' # creo il path giusto della immagine labellata R con programma Giorgio
        ...
        plt.savefig('TH+WS+LAB_valutazione.png')
