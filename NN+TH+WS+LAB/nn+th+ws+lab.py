#dopo aver predetto le immagini con la NN, questo codice applica il metodo otsu e il watershed e poi il labelling, mostra le associazioni tra "auto" e "thruth" con le frecce, calcola i valori medi e gli errori per i parametri di valutazione dell'accuratezza
...
for radius_input in range(3,4): #Viene eseguito il programma per i valori del raggio (della mediana) per i valori in range
    for min_distance_input in range(30,31): #Viene eseguito il programma per i valori della distanza (della ricerca del minimo) per i valori in range
        ...
        for element in list_file_names:
            ...
            image_local_maxima = peak_local_max(image_median, indices=False, min_distance=min_distance_input) #matrice booleana con true in corrispondenza dei massimi

            markers, nuMarkers = ndi.label(image_local_maxima) # ritorna 2 valori: markers matrice che contiene indici diversi al posto di ogni cluster di true, quantita' di regioni labellante(cluster)

            image_watershed_labels = watershed(gaussian(-image_median,12), markers, mask=binary_otsu) # riempimento delle valli dopo aver applicato filtro gaussiano e aver capovolto i massimi, comincio a riempire le valli a partire dai markers (cioe' i minimi delle valli) inoltre applico una mask per confrontare con l'immagine tresholdata con otsu la mia immagine per diminuire il rumore.
            image_watershed_labels = image_watershed_labels * binary_otsu #  per sicurezza faccio di nuovo cio' che dovrebbe aver fatto mask: moltiplico la mia immagine con l'immagine tresholdata con otsu per diminuire il rumore (cosi' lo sfondo nero che vale 0 moltiplicato per qualsiasi altro numero mi ritorna sfondo nero cioe' 0)
            ...
            S = image_watershed_labels # definisco S come l'immagine labellata watershed
            ...
            #CALCOLO J TOTALE 
            TH_labelled = (R > 0)
            matrice_moltiplicazione = (TH_labelled * (image_watershed_labels > 0))
        ...
        """
        fig, axes = plt.subplots(ncols=4, figsize=(5,3))
        axes[0] = plt.subplot(1,4,1)
        plot = plt.errorbar(x1, y1, yerr=yerr, label='radius: ')
        axes[0].imshow(plot)#+str(radius_input)+' - min distance: '+ str(min_distance_input)))
        ...
        #plt.legend('radius: ciao')


        plt.savefig('NN+TH+WS+LAB_valutazione_reflect_padding_smussed_0.4_original.png')
